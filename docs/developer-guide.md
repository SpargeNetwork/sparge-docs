...
This guide explains the HTTP boundary and implementation safeguards developers must account for when integrating with or extending a Sparge node. The [RPC API](rpc.md) is the single canonical endpoint catalogue; this guide describes shared behavior rather than duplicating endpoint definitions.
...
The accepted transaction types and canonical message are defined in [Transactions](protocol.md#transactions). Fields unused by a transaction type must be supplied as empty strings where required by the canonical payload; signed fields are never truncated or normalized by the server.
